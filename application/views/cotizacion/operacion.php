...
function _set($v, $cotizacion_info)
{
    ...
    if (isset($cotizacion_info[$v])) {
        if (is_numeric($cotizacion_info[$v]))
            return floatval($cotizacion_info[$v]);
...
<div class="panel">
    <div class="panel-heading">
        ...
        <!--        <h2 id="title-update" class="--><?php //if ($cotizacion_id <= 0) {
                                                    //            echo 'hidden';
                                                    //        } 
                                                    ?><!--">Actualizar Cotización #--><?PHP //echo $cotizacion_id; 
                                                        ?><!--</h2>-->
        <div class="pull-left">
            Currently:
    ...
    <div class="panel-body panel-form-sm">
        <form <?php if ($editable) : ?> data-trigger="custom__update_form_cotizacion" action="<?php echo site_url("cotizacion/ajax_save"); ?>" <?php else : ?> action="#" onsubmit="" <?php endif ?> method="post" class="form-horizontal js-validation-plus">
            ...
            <div class="row">
                    ...
                    <div class="well well-custom">
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                ...
                                ?>
                                <select data-rule-required="true" name="vehiculos_id" class="form-control input-sm" id="vehiculos_id" onchange="changeVehiculos(this.value)">
                                    <option value="">Seleccione Vehiculo/tipo</option>
                                    <?php
                                    $vehiculos = $this->db->get("ce_vehiculos")->result_array();
                                    ?>
                                    <?php foreach ($vehiculos as $k => $v) : ?>
                                        <option <?php if ($vehiculos_id == $v['vehiculos_id']) : ?> selected <?php endif ?> value="<?php echo $v['vehiculos_id'] ?>">
                                            <?php echo $v['name']; ?> /
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                <input data-rule-required="true" type="text" class="form-control input-sm numeric" value="<?php echo _set('kilometros', $cotizacion_info) ?>" name="kilometros" onkeyup="changeKilometer(this.value)">
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group d-flex">
                            ...
                            <div class="col-sm-6">
                                <select class="form-control input-sm" name="peso_embarque_dimension" id="peso_embarque_dimension" data-rule-required="true">
                                    <option value=""></option>
                                    <option value="TN" <?php if ($peso_embarque_dimension == 'TN') : ?> selected <?php endif ?>>Tn
                                    </option>
                                    <option value="KG" <?php if ($peso_embarque_dimension == 'KG') : ?> selected <?php endif ?>>Kg
                                    </option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-12">
                                <input data-rule-required="true" type="text" class="form-control numeric input-sm" name="importe" id="total_service" value="<?php echo _set('importe', $cotizacion_info) ?>">
                            </div>
                        </div>
                        <div class="form-group d-flex">
                            <div class="col-sm-6">
                                <input value="0" <?php
                                                    $maniobras_flag = _set('maniobras_importe', $cotizacion_info);
                                                    echo (empty($maniobras_flag)) ? '' : 'checked'; ?> type="checkbox" id="maniobras" name="maniobras">
                                <label>Incluye maniobras:</label>
                            </div>
                            <div class="col-sm-6">
                                <input id="maniobras_importe" <?php if ($is_update) : ?> <?php if (empty($maniobras_flag)) : ?> disabled <?php endif ?> <?php else : ?> enabled <?php endif ?> data-rule-required="#maniobras:checked" value="<?php echo _set('maniobras_importe', $cotizacion_info) ?>" type="text" class="form-control input-sm numeric" name="maniobras_importe">
                            </div>
                        </div>
                        <div class="form-group d-flex">
                            <div class="col-sm-6">
                                <input value="0" <?php
                                                    $custody_flag = _set('custody_importe', $cotizacion_info);
                                                    echo (empty($custody_flag)) ? '' : 'checked'; ?> type="checkbox" id="custody" name="custody">
                                <label>Incluye custody:</label>
                            </div>
                            <div class="col-sm-6">
                                <input id="custody_importe" <?php if ($is_update) : ?> <?php if (empty($custody_flag)) : ?> disabled <?php endif ?> <?php else : ?> enabled <?php endif ?> data-rule-required="#custody:checked" value="<?php echo _set('custody_importe', $cotizacion_info) ?>" type="text" class="form-control input-sm numeric" name="custody_importe">
                            </div>
                        </div>
                        <div class="form-group d-flex">
                            <div class="col-sm-6">
                                ...
                                <div>
                                    <div class='input-group date' id='datetimepicker3'>
                                        <input data-rule-required="true" value="<?php echo _set('fecha_a', $cotizacion_info) ?>" type="text" class="form-control" name="fecha_a" id="mdate1">
                            ...
                            <div class="col-sm-6">
                                ...
                                <div>
                                    <div class='input-group new date' id='datetimepicker4'>
                                        <input data-rule-required="true" value="<?php echo _set('fecha_b', $cotizacion_info) ?>" type="text" class="form-control" name="fecha_b" id="mdate">
                        ...
                        <div class="form-group">
                            <div class="col-sm-12">
                                <div>
                                    <label for="valor">
                                        <input type="checkbox" <?php $valor_no_declarado_flag = _set('declarar_valor_val', $cotizacion_info);
                                                                echo (empty($valor_no_declarado_flag)) ? '' : 'checked'; ?> value="0" id="valor_no_declarado">
                            ...
                            <div class="d-flex">
                                <div class="col-sm-6 d-flex align-items-baseline">
                                    <label>Price </label>
                                    <input id="declarar_valor_val" <?php if ($is_update) : ?> <?php if (empty($valor_no_declarado_flag)) : ?> disabled <?php endif ?> <?php else : ?> enabled <?php endif ?> data-rule-required="#valor_no_declarado_val:checked" value="<?php echo _set('declarar_valor_val', $cotizacion_info) ?>" type="text" class="form-control numeric " name="declarar_valor_val">

                                </div>
                                <div class="col-sm-6 d-flex align-items-baseline">
                                    <label>% </label>
                                    <input id="declarar_valor" <?php if ($is_update) : ?> <?php if (empty($valor_no_declarado_flag)) : ?> disabled <?php endif ?> <?php else : ?> enabled <?php endif ?> data-rule-required="#valor_no_declarado:checked" value="<?php echo _set('declarar_valor', $cotizacion_info) ?>" type="text" class="form-control numeric input-sm" name="declarar_valor">
                        ...
                        <!--                                       value="--><?php //echo _set('prima', $cotizacion_info) 
                                                                                ?><!--" type="text"-->
                ...
                <div class="col-sm-6">
                    <label>Information of Client</label>
                    ...
                    <div class="well well-custom">
                        ...
                        <!--                                       value="--><?php //echo _set('domicilio', $cotizacion_info) 
                                                                                ?><!--" type="text"-->
                        ...
                        <div class="form-group">
                            <div class="col-sm-12">
                                <div class="vehiculo-image">
                                    <img src="<?php echo base_url() ?>/uploads/vehiculos/5c511-camioneta-3.5-redilas.jpg" style="width: 100%;" id="vehiculo_logo" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <button class="btn btn-primary " data-toggle="modal" href='#modal-sendemail' type="submit" <?php echo ($editable === false) ? 'disabled' : ''; ?>> Guardar
                    </button>
                    <a href="<?php echo site_url("cotizacion/pdf/" . $cotizacion_id); ?>" target="blank" class="btn btn-primary" <?PHP if ($cotizacion_id <= 0) {
                                                                                                                                        echo "disabled";
                                                                                                                                        //                        print_r($cotizacion_id);
                                                                                                                                        //                        return;
...
<div class="form-horizontal js-validation-plus">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer d-flex justify-content-center">
                    <button type="button" class="btn btn-primary badge-pill" data-dismiss="modal">Okay</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // let id = $('#vehiculos_id').val();
        let id = document.getElementById('vehiculos_id').value;
        // console.log("kkk:" + id);
        let items = <?= json_encode($vehiculos); ?>;
        let item = items.find(val => val.vehiculos_id == id) || {};
        if (!!item) {
            var logoUrl = item.logo;
            // console.log('kkk->',logoUrl);
            vehiculo_price = item.price;
            //             $('#vehiculo_logo').attr('src', `/uploads/vehiculos/${logoUrl}`);
            const vehiculoLogo = document.getElementById('vehiculo_logo');
            if (logoUrl) vehiculoLogo.setAttribute('src', `<?php echo base_url() ?>/uploads/vehiculos/${logoUrl}`);

            //             var image = document.getElementById(#vehiculo_logo);
            //             image.src="";
        } else {
        ...
        $(document).ready(function() {
            $("#custody").on("click", function() {
                if ($(this).is(":checked")) {
                    $("#custody_importe").prop("disabled", false);
                } else {
                    $("#custody_importe").prop("disabled", true);
                }
            });
        });
        ...
        function changeVehiculos(id) {
            ...
            if (!!item) {
                var logoUrl = item.logo;
                vehiculo_price = item.price;
                $('#vehiculo_logo').attr('src', `<?php echo base_url() ?>/uploads/vehiculos/${logoUrl}`);
            } else {
                // console.log('not found');
            }
        }
        ...
        function changeKilometer(val) {
            ...
            //         console.log('total_service-->',$('#total_service').val());
            //          console.log('vehiculo_price-->',vehiculo_price);
            //           console.log('val-->',val);
        }
    </script>